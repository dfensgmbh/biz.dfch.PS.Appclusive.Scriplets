...
Describe -Tags "Catalogue.Tests" "Catalogue.Tests" {
	...
	Context "Catalogue.Tests" {
		...
		It "GettingCatalogue-Succeeds" -Test {
			...
			# Get catalogue items
			$catItems = $svc.Core.LoadProperty($cat, 'CatalogueItems') | Select;
			...
			$catItems |? Name -eq 'DSWR Office 2015 Production' | Should Not Be $null;
		}
		
		It "Adding-CartItem-CreatesCart" -Test {

			# Get catItem
			$catItem = $svc.Core.CatalogueItems |? Name -eq 'VDI Personal';
			...
			$cartItem.CatalogueItem = $catItem;
